...
if ($_GET['type'] and $psName=='CloudPayments')
{
     switch($_GET['type'])
     {
             case 'send_bill':
                    ...
                    if ($order->getPrice()>0 and $email)
                    {
                        ...
                        // echo 'http://'.$_SERVER["HTTP_HOST"].'/cloudPayments/pay.php?ORDER_ID='.$_GET['ORDER_ID'].'&hash='.$hash;
                         ...
                         echo 'Счет на сумму '.CurrencyFormat($order->getPrice(), 'RUB').' успешно отправлен!';
                    }
                                ...
                                {                                
                                    $ch = curl_init();     
                                      ...
                                      if ($data1->Success) 
                                      {
                                            ...
                                            echo 'Сумма заказа: '.CurrencyFormat($order->getPrice(), 'RUB').'<br>';
